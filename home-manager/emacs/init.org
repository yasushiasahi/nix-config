  ...
  (leaf corfu
    ...
    :hook ((corfu-mode-hook
            . (lambda ()
                ...
                (corfu-history-mode)))
           ;; ((emacs-lisp-mode-hook lisp-interaction-mode) . corfu-mode)
           )
    :config
    (leaf hotfuzz
...
#+begin_src emacs-lisp :tangle yes
  (leaf consult-omni
    ...
    (defvar my/consult-omni-web-sources (list "gptel"
                                              "Brave"
                                              "Browser History"
                                              "GitHub"
                                              "Wikipedia"                                                                                  ))
...
#+begin_src emacs-lisp :tangle yes
  (leaf eglot
    :disabled t
    ;; :hook (((yaml-ts-mode-hook html-ts-mode-hook css-ts-mode-hook astro-ts-mode) . eglot-ensure))
    :bind (:eglot-mode-map
...
#+end_src
** [[https://github.com/jadestrong/lsp-proxy][Rust製 LSPクランアント lsp-proxy]]
#+begin_src emacs-lisp :tangle yes
  (leaf lsp-proxy
    :disabled t
    :ensure t
    :hook (((jtsx-tsx-mode-hook jtsx-jsx-mode-hook jtsx-typescript-mode-hook nix-ts-mode-hook) . lsp-proxy-mode))
    :config
...
#+end_src
** 最速LSPクライアント [[https://github.com/manateelazycat/lsp-bridge][lsp-brigde]]
#+begin_src emacs-lisp :tangle yes
  (leaf lsp-bridge
    :ensure t
    :require t
    :bind (:lsp-bridge-mode-map
           ...
           ("s-l r r" . lsp-bridge-rename)
           ("s-l a" . lsp-bridge-code-action)
           ("s-l d n" . lsp-bridge-diagnostic-jump-next)
           ("s-l d p" . lsp-bridge-diagnostic-jump-prev)
           ("s-l d l" . lsp-bridge-diagnostic-jump-list)
           ("s-l d c" . lsp-bridge-diagnostic-jump-copy)
           ("C-c l r r" . lsp-bridge-rename)
           ("C-c l a" . lsp-bridge-code-action)
           ("C-c l d n" . lsp-bridge-diagnostic-jump-next)
           ("C-c l d p" . lsp-bridge-diagnostic-jump-prev)
           ("C-c l d l" . lsp-bridge-diagnostic-jump-list)
           ("C-c l d c" . lsp-bridge-diagnostic-jump-copy))
    :config
    ;;lsp-bridge.el
    (setopt lsp-bridge-nix-lsp-server "nil")
    (setopt lsp-bridge-default-mode-hooks (cons 'astro-ts-mode-hook lsp-bridge-default-mode-hooks))
    ;; lsp-bridge-org-babel.el
    (setopt lsp-bridge-enable-org-babel t)
    ;; acm.el
    (setopt acm-candidate-match-function 'orderless-flex)
    ;; (setopt acm-enable-capf t)

    (setopt lsp-bridge-user-multiserver-dir (expand-file-name "multiserver/" user-emacs-directory))
    (add-to-list 'lsp-bridge-multi-lang-server-extension-list '(("tsx") . "typescriptreact_tailwindcss_eslint"))
