...
#+end_src
** [[https://github.com/mhayashi1120/Emacs-wgrep][wgrep]] - grepの結果を編集する
#+begin_src emacs-lisp :tangle no
  (use-package wgrep
    :ensure t)
#+end_src
...
#+begin_src emacs-lisp :tangle yes
  (use-package corfu
    ...
    ;; minibuf.c
    (setopt completion-ignore-case t)     ; 候補選択時に大文字小文字を区別しない

    (global-corfu-mode)

    ;; ミニバッファ補完はordarlessなんだが、コード補完はflexにする。
...
31.1になって動かんくなった。組み込みのflexがネイティブ化されてhotfuzzと同等の早さになったらしいのでしばらくはそれで行く。
...
#+begin_src emacs-lisp :tangle yes
  (use-package astro-ts-mode
    ...
    :hook ((astro-ts-mode . my/astro-ts-mode--setup-comment))

    :config
    ;; astro-ts-modeはhtml-mode派生なのでsgml-modeのsyntax-tableを継承しており、
    ;; `.'がsymbol構成文字(`_')になっている。
    ;; lsp-proxyは(bounds-of-thing-at-point 'symbol)で補完prefixを決めるため、
    ;; `foo.'と打つとprefixが"foo."になってしまい、サーバーが返した候補が
    ;; Rust側のファジーフィルタで全て落とされて補完候補が0件になる。
    ;; `.'をpunctuationに戻してprefixが正しく計算されるようにする。
    ;; なお`-'はsymbolのままにしておく。Tailwindのクラス名やHTMLの属性名の
    ;; prefixを正しく取るにはsymbolの方が都合が良いため。
    (modify-syntax-entry ?. "." astro-ts-mode-syntax-table)

    ;; 同じ理由で`/'もsymbol構成文字になっており、import文のパス補完で
    ;; `"~/libs/'と打つとprefixが"/libs/"になり、サーバーが返した候補
    ;; (date, errorなど)がcorfuのflexフィルタで全て落とされる。
    ;; ただしHTML部分ではTailwindの`w-1/2'や`bg-black/50'のprefixを
    ;; 取るためにsymbolのままにしておきたいので、TypeScript領域
    ;; (フロントマター、<script>内)でだけ`/'をpunctuationとして
    ;; prefixを計算する。
    (defvar my/astro-ts-mode--ts-syntax-table
      (let ((st (make-syntax-table astro-ts-mode-syntax-table)))
        (modify-syntax-entry ?/ "." st)
        st)
      "TypeScript領域で補完prefixを計算するためのsyntax-table。")

    (defun my/astro-ts-mode--capf-with-ts-syntax (orig &rest args)
      "TypeScript領域では`/'をpunctuationとして補完prefixを計算する。"
      (if (and (derived-mode-p 'astro-ts-mode)
               (eq (treesit-language-at (point)) 'typescript))
          (with-syntax-table my/astro-ts-mode--ts-syntax-table
            (apply orig args))
        (apply orig args)))

    (advice-add 'lsp-proxy-completion-at-point :around
                #'my/astro-ts-mode--capf-with-ts-syntax)

    ;; comment-dwimなどでブロックごとにコメント記法を切り替える。
    ;; - フロントマター、<script>内: // (typescript)
    ;; - <style>内: /* */ (css)
    ;; - HTML部分: {/* */}
    ;; `treesit-language-at'だと`{/* ... */}'がhtml_interpolationとして
    ;; typescript扱いになりコメント解除できなくなるので、astroパーサーの
    ;; ノードで判定する。
    (defun my/astro-ts-mode--comment-lang ()
      "コメント操作の対象言語を返す。リージョンの先頭か現在行のインデント位置で判定する。"
      (save-excursion
        (if (use-region-p)
        ...
        (let ((node (treesit-node-at (point) 'astro)))
          (pcase (treesit-node-type node)
            ("frontmatter_js_block" 'typescript)
            ("raw_text"
             (if (equal (treesit-node-type (treesit-node-parent node)) "style_element")
                 'css
               'typescript))
            (_ 'astro)))))

    (defun my/astro-ts-mode--comment-setup ()
      "`comment-setup-function'用。対象の言語に合わせてコメント記法を設定する。"
      (pcase (my/astro-ts-mode--comment-lang)
        ('typescript
    ...
    (defun my/astro-ts-mode--setup-comment ()
      ;; 記法を切り替えるので構文表ではなく正規表現でコメントを判定させる
      (setq-local comment-use-syntax nil
                  comment-setup-function #'my/astro-ts-mode--comment-setup))
    )
#+end_src
...
eldoc表示時にemacs組み込みのmarkdown-ts-modeを使用する
#+begin_src emacs-lisp :tangle yes
  (use-package eglot
    :ensure t
    :init
...
#+begin_src emacs-lisp :tangle yes
  (use-package ghostel
    ...
    :init
    (setopt ghostel-shell "/run/current-system/sw/bin/fish")
    (setopt ghostel-keymap-exceptions '("C-x" "C-u" "M-x" "C-c" "M-s" "C-t"))
    (setopt ghostel-buffer-name "*👻*")
    (setopt ghostel-buffer-name-function #'my/ghostel-buffer-name-by-directory)
