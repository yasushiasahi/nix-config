...
#+begin_src emacs-lisp :tangle yes
  (leaf astro-ts-mode
    ...
    (add-to-list 'eglot-server-programs '(astro-ts-mode . ("astro-ls" "--stdio"
  																												 ...
  																												 (:typescript
  																													(:tsdk (eval (my/find-typescript-lib)))))))
    (defun my/find-typescript-lib ()
      ...
      (if-let* ((current-path  (file-name-directory (buffer-file-name)))
                ...
                (node-module-path-idx (seq-find (lambda (idx)
                                                ...
                                                idx-list))
                (node-module-parent-dir (s-join "/" (cl-subseq path-list 0 (- list-length node-module-path-idx)))))
          (concat "/" node-module-parent-dir "/typescript/lib"))))
#+end_src
** mermaid-mode
#+begin_src emacs-lisp :tangle yes
  (leaf mermaid-mode
