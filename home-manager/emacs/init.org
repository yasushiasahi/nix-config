...
#+begin_src emacs-lisp :tangle yes
  (leaf consult-omni
    ...
    (leaf browser-hist
      :ensure t
      :config
      ;; :type '(chrome chromium brave firefox safari qutebrowser) となっていてvivaldiに設定できないので、デフォルトのchromeのpathをvivaldiにする。
...
#+begin_src emacs-lisp :tangle yes
  (leaf lsp-proxy
    ...
    :bind (:lsp-proxy-mode-map
           ...
           ("C-c d" . lsp-proxy-describe-thing-at-point))
    ...
    (setopt lsp-proxy-log-max 0)
    (setopt lsp-proxy-log-buffer-max nil)
    (setopt lsp-proxy-log-level 0)

    ;; (setopt lsp-proxy-log-max 100000)
